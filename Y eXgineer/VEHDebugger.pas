...
resourcestring
  ...
  rsYeXgineerFailedToGetIntoTheConfig = 'Y eXgineer failed to get into '
  ...
  try
   ...
   { if symhandler.getmodulebyname('vehdebug'+prefix+'.dll',mi) then
    ...
    begin
      ...
      raise exception.Create(Format(rsYeXgineerFailedToGetIntoTheConfig, [
        inttostr(e)]));
    end;
    ...
    begin
      try
        InjectDll(yeXgineerdir+'vehdebug'+prefix+'.dll');
      except
      end;
    end;
