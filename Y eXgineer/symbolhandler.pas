...
begin
  ...
  try
    ...
    symbolpath:=usedtempdir+'Y eXgineer Symbols'+pathdelim;
...
var
  ...
  i,j: integer;
begin
  s:=yeXgineerdir+'commonmodulelist.txt';
...
{$ifdef windows}
{$ifdef cpu32}
  dbghlp:=LoadLibrary(pchar(YeXgineerDir+'\win32\dbghelp.dll'));
{$else}
  dbghlp:=LoadLibrary(pchar(YeXgineerDir+'\win64\dbghelp.dll'));
{$endif}
...
begin
  ...
  databasepath:=usedtempdir+'Y eXgineer Symbols'+pathdelim+'structures.sqlite';
end;
