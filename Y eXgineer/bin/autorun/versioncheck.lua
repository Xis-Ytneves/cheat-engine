--version check update script for y eXgineer
...
function CheckVersion(automatic)
  ...
  if versionCheckThread==nil then
    versionCheckThread=createThread(function(t)
        ...
        local r=i.getURL('https://eXgineer.org/latestversion.txt')
        ...
        if r then
          ...
          local fv=getYeXgineerFileVersion()
          ...
          t.synchronize(function()
            if newerVersion then
              if messageDialog(string.format(translate('Y eXgineer %s is available at www.eXgineer.org. Go there now?'),latestVersionString), mtConfirmation, mbYes, mbNo)==mrYes then
                shellExecute('https://eXgineer.org/?versioncheck=1')
              else
...
cbCheckForUpdatesOnLaunch.Caption=translate('Check for updates when Y eXgineer starts')..'.'
