...
{
Modified for Y eXgineer use because filenames differ between 32 and 64-bit (also different po file, but just pick the one with the most data...)
...
var
  ...
  begin
    ...
    begin
      ...
      Result := yeXgineerdir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'eXgineer'+LCEXT;
      if FileExists(Result) then exit;

      Result := yeXgineerdir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'eXgineer-x86_64'+LCEXT;
      if FileExists(Result) then exit;

      Result := yeXgineerdir + 'languages' + DirectorySeparator + LangID + DirectorySeparator + 'eXgineer-i386'+LCEXT;
      if FileExists(Result) then exit;
...
begin
  ...
  if (lang = '') and (FileExists(yeXgineerdir+ 'languages' + DirectorySeparator+'language.ini')) then
  begin
    try
      lini:=TIniFile.Create(yeXgineerdir+'languages' + DirectorySeparator+'language.ini');
      try
