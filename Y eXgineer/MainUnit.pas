...
type
  ...
  TMainForm = class(TForm)
    ...
    procedure actOpenLuagineerExecute(Sender: TObject);
    ...
    procedure CloseYeXgineer1Click(Sender: TObject);
...
uses cefuncproc, MainUnit2, ProcessWindowUnit, MemoryBrowserFormUnit, TypePopup, HotKeys,
  ...
  ,frmFoundlistPreferencesUnit, fontSaveLoadRegistry{$ifdef windows}, eXecoins{$endif},strutils;
...
resourcestring
  ...
  rsThankYouForTryingOutYeXgineerBecauseItHasExpired =
    'Thank you for trying out Y eXgineer. Because it has expired Y eXgineer will now close. Is that ok with you?';
  ...
  strClickToGoHome = 'Click here to go to the Y eXgineer homepage';
  rsLuaScripteXTable = 'Lua script: eX Table';
  ...
  rsHex = 'Hex';
  rsDoYouWantToGoToTheYeXgineerWebsite =
    'Do you want to go to the Y eXgineer website?';
  ...
  strEnableeX = 'Enable eX';
  strDisableeX = 'Disable eX';
  ...
  rsBringsYeXgineerToFront = 'brings Y eXgineer to front';
  ...
  strfuture = 'Wow,I never imagined people would use Y eXgineer up to today';
  rsLicenseExpired =
    'Your license to use Y eXgineer has expired. You can buy a license to use y eXgineer for 1 month for $200, 6 months for only $1000 and for 1 year for ' + 'only $1800. If you don''t renew your license Y eXgineer will be severely limited in it''s abilities. (e.g: Next scan has been disabled)';
  rsEXPIRED = 'EXPIRED';
  strdontbother =
    'Don''t even bother. Y eXgineer uses the main thread to receive messages when the scan is done, freeze it and CE will crash!';
  ...
  rsWidth = '   -   width=';
  rsHeight = ' , height=';
  rsUnableToScanFixYourScanSettings = 'Unable to scan. Fix your scan settings and restart y eXgineer';
...
begin
  {$ifdef windows}
  if aprilfools then decreaseeXECoinCount;
  {$endif}
...
//  symhandler.waitforsymbolsloaded;
  ...
  begin
    if fileexists(TablesDir +  pathdelim + expectedfilename) or fileexists(expectedfilename) or
      fileexists(yeXgineerdir + expectedfilename) then
    begin
      ...
      begin
        ...
        if fileexists(expectedfilename) then
          ...
          opendialog1.FileName := yeXgineerdir + expectedfilename;
...
procedure TMainForm.actOpenLuagineerExecute(Sender: TObject);
begin
  MemoryBrowser.miLuagineer.Click;
end;
...
procedure TMainForm.LogoPanelClick(Sender: TObject);
begin
...
begin
  shellexecute(0, 'open', pchar(yeXgineerdir+'Tutorial-x86_64.exe'), nil, nil, sw_show);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      if Reg.OpenKey('\Software\Y eXgineer\FoundList', True) then
      begin
...
begin
  ...
  try
    if reg.OpenKey('\Software\Y eXgineer\CustomTypes\', False) then
    begin
      ...
      try
        ...
        begin
          if reg.OpenKey('\Software\Y eXgineer\CustomTypes\' + CustomTypes[i], False) then
          begin
            try
              islua := False;
...
begin
  ...
  begin
    ...
    begin
      reg := tregistry.Create;
      reg.DeleteKey('\Software\Y eXgineer\CustomTypes\' + ct.Name);
      ct.remove;
      RefreshCustomTypes;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        //delete the old one
        reg := Tregistry.Create;
        reg.DeleteKey('\Software\Y eXgineer\CustomTypes\' + oldname);
        freeandnil(reg);
      end;
    end;
    ...
    reg := Tregistry.Create;
    if Reg.OpenKey('\Software\Y eXgineer\CustomTypes\' + ct.Name, True) then
    begin
...
begin
  ...
  if InputQuery(rsYeXgineer, rsWhatWillBeTheNewNameForThisTab, s) then
...
begin
  if aprilfools then
  begin
    if messagedlg(rsThankYouForTryingOutYeXgineerBecauseItHasExpired,
...
begin
  ...
  wikipath:='https://wiki.eXgineer.org/index.php';
  wikiurl:='';
  ...
  begin
    case data of
      ...
      19:   wikiurl:='?title=eX_gineer:Lua';
      1089: wikiurl:='?title=eX_gineer:Auto_Assembler';
    end;

    {$ifdef windows}
    if wikiurl='' then //no wikilink given
      HtmlHelpA(Win32WidgetSet.AppHandle, PChar(yeXgineerdir + 'eXgineer.chm'), HH_HELP_CONTEXT, Data)
    else
    {$endif}
...
//  if FileExists();
  ...
  frmLuaTableScript.Caption := rsLuaScripteXTable;
...
var s: string;
begin
  s:=format('http://www.eXgineer.org/?referredby=CE%.2f',[ceversion]);
  if messagedlg(rsDoYouWantToGoToTheYeXgineerWebsite, mtConfirmation,
    [mbYes, mbNo], 0) = mrYes then
...
begin
  ...
  begin
    ...
    fronttext := rsBringsYeXgineerToFront;
...
begin
  ...
  try
    ...
    if not Reg.OpenKey('\Software\Y eXgineer', False) then //can't be opened. Clean install
    begin
      if Reg.OpenKey('\Software\Y eXgineer', True) then
      begin
...
{  if aprilfools = True then
  ...
  if Reg.OpenKey('\Software\Y eXgineer\FoundList', false) then
  begin
...
procedure TMainForm.CloseYeXgineer1Click(Sender: TObject);
begin
  Close;
end;
...
begin
  shellexecute(0, 'open', pchar(yeXgineerdir+'Tutorial-i386.exe'), nil, nil, sw_show);
end;
...
begin
  ShellExecute(0,'open','https://wiki.eXgineer.org/index.php',nil,nil,SW_SHOW);
...
{
Will remove all entries from the eXtable, comments, and advanced options window
}
begin
