...
ceshare.version=-1
ceshare.path=getYeXgineerDir()..[[autorun\ceshare\]]
ceshare.formpath=ceshare.path..[[\forms\]]

if package.loaded.xmlSimple==nil then
  package.path=package.path..';'..getYeXgineerDir()..[[autorun\xml\?.lua]]
else
...
function loadCEShare()
  ...
  local miCheckForeXs=createMenuItem(MainForm)
  miCheckForeXs.Caption='Check for mods/eXs for current process'
  miCheckForeXs.OnClick=ceshare.CheckForeXsClick
  miCheckForeXs.Default=true
  miCheckForeXs.Name='miCheckForeXs'
  miCESHARETopMenuItem.add(miCheckForeXs)
  
  local miRequesteXs=createMenuItem(MainForm)
  miRequesteXs.Caption='Request and check other requests for mods/eXs for current process'
  miRequesteXs.OnClick=ceshare.RequestForeXsClick
  miRequesteXs.Name='miRequesteXs'
  miCESHARETopMenuItem.add(miRequesteXs)  

  local miPublisheX=createMenuItem(MainForm)
  miPublisheX.Caption='Publish/Update table'
  miPublisheX.OnClick=ceshare.PublisheXClick
  miPublisheX.Name='miPublisheX';
  miPublisheX.Visible=false
  miCESHARETopMenuItem.add(miPublisheX)

  --check requestsc
  
  miCESHARETopMenuItem.OnClick=function(s)      
    loggedin=ceshare.LoggedIn or false
    miPublisheX.Visible=true
...
--]]
  ...
  if ceshare.ceshareserverlist==nil then
    local list=ceshare.getInternet().getURL('https://eXgineer.org/cesharelist.txt') --AKA: Eggbaskets
