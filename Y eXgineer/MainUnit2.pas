    ...
    try
      ...
      begin
        ...
        begin
          ...
          if reg.ValueExists('Show Y eXgineer Hotkey') then
            {$ifdef windows}
            reg.ReadBinaryData('Show Y eXgineer Hotkey',temphotkeylist[1][0],10);
            {$else}
            HexToBin(pchar(reg.ReadString('Show Y eXgineer Hotkey')),pchar(@temphotkeylist[1][0]),10);
            {$endif}
      ...
      if Reg.OpenKey('\Software\Y eXgineer\Tools',false) then
      begin
      ...
      if (not skipPlugins) and (Reg.OpenKey('\Software\Y eXgineer\Plugins'{$ifdef cpu64}+'64'{$else}+'32'{$endif},false)) then
      begin
        names:=TStringList.create;
        try
