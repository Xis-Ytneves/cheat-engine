...
begin
  try
    f:=TFilestream.Create(YeXgineerDir+'accesscheck.tmp', fmCreate);
    try
  ...
  try
    f:=TFilestream.Create(YeXgineerDir+'accesscheck.tmp', fmOpenReadWrite);
    try
      f.Seek(0,soFromEnd);
  ...
  if not deletefile(YeXgineerDir+'accesscheck.tmp') then
