...
//Y eXgineer regular WONT look at this
...
begin
  assignfile(memoryfile,YeXgineerDir+'Memory.TMP');
  assignfile(addressfile,YeXgineerDir+'Addresses.TMP');
  reset(memoryfile,1);
  reset(addressfile,1);

  assignfile(newmemoryfile,YeXgineerDir+'Memory2.TMP');
  assignfile(newaddressfile,YeXgineerDir+'Address2.TMP');
  ...
  deletefile(YeXgineerDir+'Memory.UNDO');
  deletefile(YeXgineerDir+'Addresses.UNDO');
  renamefile(YeXgineerDir+'Memory.tmp',yeXgineerdir+'Memory.UNDO');
  renamefile(YeXgineerDir+'Addresses.tmp',YeXgineerDir+'Addresses.UNDO');
  renamefile(YeXgineerDir+'Memory2.tmp',YeXgineerDir+'Memory.TMP');
  Renamefile(yeXgineerDir+'Address2.TMP',YeXgineerDir+'Addresses.TMP');
...
//http://forum.eXgineer.org/viewtopic.php?t=564382
{$IFDEF windows}
type
...
{$ENDIF}
begin
  YeXgineerDir:=ExtractFilePath(application.ExeName);
  result:=YeXgineerDir;
  ...
  if SHGetPathFromIDList(PIDL, Path) then
    tablesdir := WinCPToUTF8(Path)+'\My eX Tables';
  SHGetMalloc(AMalloc);
  AMalloc.Free(PIDL);
  StrDispose(Path);
...
begin
  deletefile(YeXgineerDir+'Memory.TMP');
  deletefile(YeXgineerDir+'Addresses.TMP');
  deletefile(YeXgineerDir+'Memory.UNDO');
  deletefile(YeXgineerDir+'Addresses.UNDO');
  freemem(memory);
  memory:=nil;
...
begin
  ...
  try
    ...
    try
      ...
      if Reg.OpenKey('\Software\Y eXgineer',false) then
      begin
        if reg.valueexists('Save window positions') then
          if reg.readbool('Save window positions') = false then exit;
      end;

      if Reg.OpenKey('\Software\Y eXgineer\Window Positions '+inttostr(screen.PixelsPerInch),false) or Reg.OpenKey('\Software\Y eXgineer\Window Positions',false) then
      begin
        s:=form.Name;
        s:=s+rsPosition;
...
begin
  ...
  begin
    ...
    try
      ...
      if Reg.OpenKey('\Software\Y eXgineer\Window Positions '+inttostr(screen.PixelsPerInch),true) then
      begin
...
begin
  result:=filename;
  if pos(uppercase(YeXgineerDir),uppercase(filename))=1 then
    result:='.\'+copy(filename,length(YeXgineerdir)+1,length(filename));
end;
