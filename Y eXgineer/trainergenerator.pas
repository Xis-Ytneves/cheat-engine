...
type
  ...
  TfrmTrainerGenerator = class(TForm)
    ...
    procedure cbSupportYeXgineerChange(Sender: TObject);
    ...
    procedure lveXsDblClick(Sender: TObject);
    procedure lveXsSelectItem(Sender: TObject; Item: TListItem;
      Selected: Boolean);
...
resourcestring
  ...
  rsTo = 'to';
  rsBy = 'by';
  rsOnCloseWarning = 'This form had an onClose event. Good thing this was only a stub, else Y eXgineer would have terminated';
  rsAlreadyATrainerFormDefined =
      'There is already a trainer form defined. '
    +'Continuing will erase the current trainerscript and eXs in the '
    +'trainer and replace them with the current hotkeys defined in your '
    +'current eX table (Layout and images will remain unchanged). Continue ?';
  rsNoeXPanel = 'The current trainer form does not have a panel '
    +'named ''EXPANEL'' so can not be reused by the automated trainer '
  ...
  rseXEntries = 'eX Entries';
  rsSelectTheeXEntryYouWantToSetTheHotkeyFor = 'Select the eX entry you '
    +'want to set the hotkey for';
  rsYouNeedAeXTableWitheXEntries = 'You need a eX table with eX '
    +'entries';
  rsDonTSupportYeXgineerOrYourself = 'Don''t support Y eXgineer (or '
    +'yourself)';
...
procedure TfrmTrainerGenerator.buildeXlist;
var eXpanel: TCEPanel;
  i: integer;
  currenteX, lasteX: TeX;

  hk: TMemoryRecordHotkey;
begin
  eXpanel:=TCEPanel(trainerform.FindComponent('EXPANEL'));

  if eXpanel<>nil then
  begin
    //clear the old list (only the TeX objects)
    i:=0;
    while i<eXpanel.ControlCount do
    begin
      if eXpanel.controls[i] is teX then
        eXpanel.Controls[i].Free
      else
        inc(i);
    end;
    ...
    currenteX:=nil;
    for i:=0 to lveXs.Items.Count-1 do
    begin
      lasteX:=currenteX;
      currenteX:=teX.create(trainerform);
      currenteX.parent:=eXpanel;
      currenteX.name:='eX'+inttostr(i);
      currenteX.eXnr:=i;
      currenteX.AutoSize:=true;
      ...
      begin
        //next one
        currenteX.top:=lasteX.Top+lasteX.height+10;
        currenteX.left:=lasteX.left;
      end;

      currenteX.hotkeyleft:=hotkeylabel.left-currenteX.left;
      currenteX.descriptionleft:=descriptionlabel.left-currenteX.left;

      currenteX.width:=eXpanel.clientwidth-currenteX.Left-2;
      currenteX.anchors:=currenteX.anchors+[akRight];

      currenteX.Hotkey:=lveXs.Items[i].Caption;
      currenteX.Description:=lveXs.Items[i].SubItems[0];
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if eXpanel=nil then
        begin
          if messagedlg(Format(rsNoeXPanel, [#13#10]), mtError, [mbyes, mbno], 0)=mryes then
            trainerform:=nil;
  ...
  begin
    ...
    eXpanel:=Tcepanel.create(trainerform);
    eXpanel.align:=alclient;
    eXpanel.name:='EXPANEL';
    eXpanel.caption:='';
    eXpanel.parent:=trainerform;
    ...
    //these are part of the eXpanel which has ben destroyed or just created
    hotkeylabel:=Tcelabel.create(trainerform);
    hotkeylabel.name:='HOTKEYLABEL';
    hotkeylabel.caption:=rsHotkey;
    hotkeylabel.left:=10;
    hotkeylabel.top:=10;
    hotkeylabel.parent:=eXpanel;
    ...
    descriptionlabel.parent:=eXpanel;


    closebutton:=TCEButton.create(trainerform);
    closebutton.name:='CLOSEBUTTON';
    closebutton.caption:=rsClose;
    closebutton.top:=eXpanel.clientheight - closebutton.height-8;
    closebutton.left:=eXpanel.clientwidth div 2 - closebutton.width div 2;
    closebutton.parent:=eXpanel;
...
procedure TfrmTrainerGenerator.lveXsDblClick(Sender: TObject);
begin
  miEditHotkey.Click;
end;

procedure TfrmTrainerGenerator.lveXsSelectItem(Sender: TObject;
...
begin
  ...
  if selected then
  begin
    spbDown.enabled:=item.index<lveXs.items.count-1;
...
var mh: TMemoryrecordhotkey;
  mr: TMemoryrecord;
begin
  if lveXs.selected<>nil then
  begin
    mh:=TMemoryrecordhotkey(lveXs.selected.data);
    mr:=mh.owner;
    editHotkey(mr,mh);

    RefreshHotkeyItem(lveXs.selected);
  end;
end;
...
begin
  lveXs.Column[1].Width:=lveXs.clientwidth-lveXs.Column[0].Width-3;
end;
...
begin
  if lveXs.ItemIndex<>-1 then
  begin
    if lveXs.ItemIndex<lveXs.items.count-1 then
    begin
      temphotkey:=lveXs.items[lveXs.ItemIndex].Caption;
      tempdescription:=lveXs.items[lveXs.ItemIndex].SubItems[0];
      tempdata:=lveXs.items[lveXs.ItemIndex].data;

      lveXs.items[lveXs.ItemIndex].Caption:=lveXs.items[lveXs.ItemIndex+1].Caption;
      lveXs.items[lveXs.ItemIndex].subitems[0]:=lveXs.items[lveXs.ItemIndex+1].subitems[0];
      lveXs.items[lveXs.ItemIndex].data:=lveXs.items[lveXs.ItemIndex+1].data;

      lveXs.items[lveXs.ItemIndex+1].Caption:=temphotkey;
      lveXs.items[lveXs.ItemIndex+1].subitems[0]:=tempdescription;
      lveXs.items[lveXs.ItemIndex+1].data:=tempdata;

      lveXs.itemindex:=lveXs.itemindex+1;
...
begin
  if lveXs.ItemIndex>=1 then
  begin
    temphotkey:=lveXs.items[lveXs.ItemIndex-1].Caption;
    tempdescription:=lveXs.items[lveXs.ItemIndex-1].SubItems[0];
    tempdata:=lveXs.items[lveXs.ItemIndex-1].data;

    lveXs.items[lveXs.ItemIndex-1].Caption:=lveXs.items[lveXs.ItemIndex].Caption;
    lveXs.items[lveXs.ItemIndex-1].subitems[0]:=lveXs.items[lveXs.ItemIndex].subitems[0];
    lveXs.items[lveXs.ItemIndex-1].data:=lveXs.items[lveXs.ItemIndex].data;

    lveXs.items[lveXs.ItemIndex].Caption:=temphotkey;
    lveXs.items[lveXs.ItemIndex].subitems[0]:=tempdescription;
    lveXs.items[lveXs.ItemIndex].data:=tempdata;
...
begin
  ...
  if not cbSupportYeXgineer.checked then
    cbSupportYeXgineer.checked:=true;
...
begin
  ...
  begin
    ...
    begin
      hk:=TMemoryRecordHotkey(lveXs.selected.data);
      hk.Free;

      lveXs.Selected.Delete;
...
begin
  ...
  l.add('');
  l.add('--Uncomment the following line if this is a eX Table format trainer and you don''t want CE to show (Tip, save as .CETRAINER alternatively)');
  ...
  l.add('  messageDialog(''Please install Y eXgineer ''..RequiredCEVersion, mtError, mbOK)');
  l.add('  closeCE()');
  l.add('end');
  ...
  try
    ...
    if cbUseD3DHook.checked then
      l.add('d3deXs={}  --table containing the information to build the eX lines for d3d');
    ...
    begin
      ...
      //create the routines for these eXs
      ...
      for i:=0 to lveXs.Items.Count-1 do
      begin
        currenthk:=TMemoryRecordHotkey(lveXs.Items[i].Data);
      ...
      //now go through the actual eXlist on the form itself and give it it's functions


      for i:=0 to eXpanel.ControlCount-1 do
      begin

        if eXpanel.Controls[i] is TeX then
        begin
          currenteX:=TeX(eXpanel.Controls[i]);


          currenthk:=TMemoryRecordHotkey(lveXs.Items[currenteX.eXnr].Data);
          currentmr:=currenthk.owner;

          if cbUseD3DHook.checked then
          begin
            l.add('d3deXs['+inttostr(currenteX.eXnr+1)+']={}');
            l.add('d3deXs['+inttostr(currenteX.eXnr+1)+'].description=[['+currenteX.Description+']]');
            l.add('d3deXs['+inttostr(currenteX.eXnr+1)+'].hotkeys=[['+currenteX.Hotkey+']]');
            l.add('d3deXs['+inttostr(currenteX.eXnr+1)+'].top='+inttostr(currenteX.Top));
            l.add('d3deXs['+inttostr(currenteX.eXnr+1)+'].left='+inttostr(currenteX.Left));
            l.add('d3deXs['+inttostr(currenteX.eXnr+1)+'].memrecid='+inttostr(currentmr.id));
            l.add('d3deXs['+inttostr(currenteX.eXnr+1)+'].hotkeyid='+inttostr(currenthk.id));
          ...
          case currenthk.action of
            ...
            begin
              //constantly enabled
              fname:='onPostHotkey'+inttostr(currenteX.eXnr);
              l.Add('function '+fname+'(Hotkey)');
              l.add('  --Executed after the "toggle*" eX got executed');
              l.add('  local memrec=Hotkey.Owner');
              l.add('  local isActive=memrec.Active --get the state after the hotkey got triggered');
              l.add('  '+trainerform.name+'.'+currenteX.name+'.setActive(isActive) --gui update, nothing else');
              ...
              begin
                ...
                l.add('  end');
                l.add('  d3deXs['+inttostr(currenteX.eXnr+1)+'].CheckboxSprite.Texture=newcbtexture');
              end;
              l.add('end');
              l.add('');
            ...
            begin
              //one time only
              fname:='onHotkey'+inttostr(currenteX.eXnr);

              l.Add('function '+fname+'(Hotkey)');
              l.add('  --Executed before the hotkey is handled');
              if currenteX.HasEditBox then
              begin
                l.add('  local memrec=Hotkey.Owner');
                l.add('  memrec.Value='+trainerform.name+'.'+currenteX.name+'.Editvalue --this will also update the ''frozen'' value');
              end;
              l.add('  '+trainerform.name+'.'+currenteX.name+'.setActive(true, 1500)');
              l.add('  if gPlaySoundOnAction then');
    ...
    else //the user deleted the eX panel
    ...
    begin
      ...
      begin
        ...
        l.add('supportYeXgineer('+trainerform.name+', '+BoolToStr(adconfig.cbCanClose.checked,'true','false')+', '+adconfig.edtWidth.text+', '+adconfig.edtHeight.text+', '+inttostr(adconfig.adposition)+', '+QuotedStr(adconfig.ownurl)+', '+QuotedStr(adconfig.extraparam)+', '+inttostr(adconfig.percentage)+')');
    ...
    begin
      ...
      begin
        ...
        l.add('');
        l.add('    --create the eX entry lines');
        l.add('    for i,info in ipairs(d3deXs) do');
        ...
        l.add('      --clicked on a eX entry. Execute the hotkey event');
        ...
        l.add('  --set the background position and go through the d3deXs. Optionally also stretching the background sprite');
        l.add('  local maxX=0');
...
begin
  ...
  case cbOutput.ItemIndex of
    ...
    begin
      ...
      begin
        messagedlg('You may not name your trainer .EXE the same as the process you wish to eX on', mtError, [mbok],0);
        exit;
      end;
...
begin
  ...
  begin
    ...
    begin
      //delete from lveXs
      for j:=0 to lveXs.items.count-1 do
        if lveXs.items[j].Data=oldlist[i] then
        begin
          //found it
          lveXs.Items.Delete(j);
          break;
        end;
  ...
  for i:=0 to lveXs.items.count-1 do
    RefreshHotkeyItem(lveXs.items[i]);
...
begin
  ...
  begin
    ...
    l.Caption:=rseXEntries;
    l.label1.caption:=rsSelectTheeXEntryYouWantToSetTheHotkeyFor;
    l.itemindex:=0;
    ...
    raise exception.create(rsYouNeedAeXTableWitheXEntries);
end;
...
begin
  cbSupportYeXgineer.caption:=rsDonTSupportYeXgineerOrYourself;
  if restoretimer<>nil then
    restoretimer.enabled:=false;
end;

procedure TfrmTrainerGenerator.cbSupportYeXgineerChange(Sender: TObject);
begin
  //Guilt procedure
  if not cbSupportYeXgineer.checked then
  begin
    cbSupportYeXgineer.caption:=rsThankYou;
  ...
  begin
    cbSupportYeXgineer.caption:=rsAaaaw;
    if adwindow<>nil then
    begin
