...
resourcestring
  ...
  rsYouHavenTSelectedAnyMemoryTypeThisWillResultInChea = 'You haven''t selected any memory type. This will result in Y eXgineer finding NO memory! Are you stupid?';
  ...
  rsPopupHideYeXgineer = 'Popup/Hide y eXgineer';
  ...
  rsRestartCE = 'It is recommended to restart Y eXgineer for this change to take effect';
...
begin
    ...
    try
      ...
      begin
        ...
        try
          ...
          reg.{$ifdef windows}WriteBinaryData{$else}WriteString{$endif}('Show Y eXgineer Hotkey',{$ifndef windows}bintohexs({$endif}frameHotkeyConfig.newhotkeys[1][0],10){$ifndef windows}){$endif};
        ...
        begin
          if reg.OpenKey('\Software\Y eXgineer\Font', true) then
  ...
  {$ifndef net}
      ...
      reg.DeleteKey('\Software\Y eXgineer\Tools');
      if Reg.OpenKey('\Software\Y eXgineer\Tools',true) then
      begin
      ...
      //save the plugins
      reg.DeleteKey('\Software\Y eXgineer\Plugins'+cpu);
      if Reg.OpenKey('\Software\Y eXgineer\Plugins'+cpu,true) then
      begin
...
begin
  ...
  begin
    ...
    try
      ini:=TIniFile.Create(yeXgineerdir+'languages' + DirectorySeparator+'language.ini');
      try
...
begin
  ...
  FindAllDirectories(f,YeXgineerDir+'\languages',false);
  for i:=0 to f.Count-1 do
  begin
    n:=f[i];
    if not (fileexists(n+'\eXgineer.po') or fileexists(n+'\eXgineer-x86_64.po') or fileexists(n+'\eXgineer-i386.po')) then
      continue;
