...
procedure ReloadAllLuagineerHighlighters;

var
  frmLuagineer: TfrmLuagineer;
...
resourcestring
  ...
  rsLuagineer = 'Lua gineer';

var
  LuaDebugForm: TfrmLuagineer;
...
var
  i: integer;
  f: TCustomForm;
  lef: TfrmLuagineer absolute f;
begin
  for i:=0 to screen.FormCount-1 do
  begin
    f:=screen.Forms[i];
    if f is TfrmLuagineer then
      lef.reloadHighlighterSettings;
  end;

end;

procedure TfrmLuagineer.Panel2Resize(Sender: TObject);
begin
  btnexecute.Height:=panel2.clientheight-(2*btnexecute.top);
end;

procedure TfrmLuagineer.ContinueAutoComplete;
var p,p2: tpoint;
begin
...
procedure TfrmLuagineer.ContinueAutoComplete2(sender: TObject);
begin
...
procedure TfrmLuagineer.scLuaCompleterCodeCompletion(var Value: string;
...
procedure TfrmLuagineer.scLuaCompleterExecute(Sender: TObject);
var
  s,extra: string;
  w: tpoint;
  i,j,si: integer;
...
procedure TfrmLuagineer.scLuaCompleterKeyCompletePrefix(Sender: TObject);
begin

end;

procedure TfrmLuagineer.scLuaCompleterPositionChanged(Sender: TObject);
begin

end;

procedure TfrmLuagineer.scLuaCompleterSearchPosition(var APosition: integer);
var
  s,s2: string;
  i,p: integer;
  start: integer;
...
procedure TfrmLuagineer.SQLConnector1AfterConnect(Sender: TObject);
begin

end;

procedure TfrmLuagineer.tbRunClick(Sender: TObject);
begin
...
procedure TfrmLuagineer.tbSingleStepClick(Sender: TObject);
begin
...
procedure TfrmLuagineer.tbStopDebugClick(Sender: TObject);
begin
...
procedure TfrmLuagineer.tShowHintTimer(Sender: TObject);
var r: trect;
  description: string;
  p,p2,p3: tpoint;
  token: string;
...
begin
  ...
  begin
    ...
    begin
      //break
     // frmLuagineer.visible:=false;
     // frmLuagineer.ShowModal;
     // frmLuagineer.show;
...
//    frmLuagineer.moutput.lines.add('called:'+ar.what+' ('+inttostr(ar.currentline)+')');
...
procedure TfrmLuagineer.btnExecuteClick(Sender: TObject);
var pc: pchar;
  i,j,ln: integer;
...
procedure TfrmLuagineer.cbShowOnPrintClick(Sender: TObject);
begin

end;

procedure TfrmLuagineer.dlgReplaceFind(Sender: TObject);
...
procedure TfrmLuagineer.dlgReplaceReplace(Sender: TObject);
var so: TSynSearchOptions;
begin
  so:=[];
  if not (frDown in dlgReplace.Options) then
...
procedure TfrmLuagineer.FormCreate(Sender: TObject);
var
  x: array of integer;
  fq: TFontQuality;
begin
...
procedure TfrmLuagineer.FormDestroy(Sender: TObject);
...
procedure TfrmLuagineer.FormShow(Sender: TObject);
var i: integer;
begin
...
procedure TfrmLuagineer.miUndoClick(Sender: TObject);
begin
  mscript.Undo;
end;

procedure TfrmLuagineer.MenuItem11Click(Sender: TObject);
var f: TfrmLuagineer;
begin
  f:=TfrmLuagineer.create(application);
  f.miView.visible:=false;

  f.show;
end;

procedure TfrmLuagineer.miFindClick(Sender: TObject);
begin
  finddialog1.Execute;
end;

procedure TfrmLuagineer.miRedoClick(Sender: TObject);
begin
  mscript.redo;
end;

procedure TfrmLuagineer.reloadHighlighterSettings;
begin
  synhighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\Y eXgineer\Lua Highlighter');
end;

procedure TfrmLuagineer.MenuItem15Click(Sender: TObject);
var
...
begin
  ...
  begin
    synhighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\Y eXgineer\Lua Highlighter');
    ReloadAllAutoInjectHighlighters; //AA uses lua too
    ReloadAllLuagineerHighlighters;
  end;

  frmHighlighterEditor.free;
end;

procedure TfrmLuagineer.MenuItem2Click(Sender: TObject);
begin
...
procedure TfrmLuagineer.MenuItem3Click(Sender: TObject);
begin
...
procedure TfrmLuagineer.MenuItem5Click(Sender: TObject);
begin
  moutput.Clear;
end;

procedure TfrmLuagineer.miFindReplaceClick(Sender: TObject);
begin
  dlgReplace.Execute;
end;

procedure TfrmLuagineer.miCutClick(Sender: TObject);
begin
  mscript.CutToClipboard;
end;

procedure TfrmLuagineer.miCopyClick(Sender: TObject);
begin
  mscript.CopyToClipboard;
end;

procedure TfrmLuagineer.miPasteClick(Sender: TObject);
begin
  mscript.PasteFromClipboard;
end;

procedure TfrmLuagineer.miResizeOutputClick(Sender: TObject);
begin
...
procedure TfrmLuagineer.miSaveCurrentScriptAsClick(Sender: TObject);
begin
  if savedialog1.Execute then
  begin
    mscript.lines.SaveToFile(savedialog1.filename);
    frmLuagineer.Caption:=rsLuagineer+' '+ExtractFileNameOnly(savedialog1.filename);
  end;
end;

procedure TfrmLuagineer.miSetBreakpointClick(Sender: TObject);
begin
  mScriptGutterClick(mScript, 0,0, mscript.CaretY, nil);
end;

procedure TfrmLuagineer.miShowScriptInOutputClick(Sender: TObject);
begin

end;

procedure TfrmLuagineer.mScriptChange(Sender: TObject);
begin

end;

procedure TfrmLuagineer.mScriptGutterClick(Sender: TObject; X, Y,
...
procedure TfrmLuagineer.mScriptKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmLuagineer.mScriptKeyPress(Sender: TObject; var Key: char);
var p,p2: tpoint;
begin
...
procedure TfrmLuagineer.mScriptMouseEnter(Sender: TObject);
begin
  tShowHint.enabled:=false;
  tShowHint.AutoEnabled:=true;
end;

procedure TfrmLuagineer.mScriptMouseLeave(Sender: TObject);
begin
  tShowHint.AutoEnabled:=false;
  tShowHint.enabled:=false;
end;

procedure TfrmLuagineer.mScriptMouseLink(Sender: TObject; X, Y: Integer;
  var AllowMouseLink: Boolean);
begin
  AllowMouseLink:=false;
end;

procedure TfrmLuagineer.mScriptMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if hintwindow<>nil then
    hintwindow.hide;
...
procedure TfrmLuagineer.mScriptShowHint(Sender: TObject; HintInfo: PHintInfo);
begin
