...
begin
  ...
  begin
    ...
    MainForm.frmLuaTableScript.assemblescreen.Lines.Insert(2, '  messageDialog(''Please install Y eXgineer ''..RequiredCEVersion, mtError, mbOK)');
  ...
  try
    ...
    if CopyFile(yeXgineerdir+basefile+'.dat', filename) then
    begin
      ...
      begin
        ...
        begin
          ...
          decompressor.LoadFromFile(yeXgineerdir+'standalonephase2.dat');
          ...
          addfile(yeXgineerdir+'defines.lua');

          if rb32.checked then
          begin
            addfile(yeXgineerdir+'eXgineer-i386.exe');

            addfile(yeXgineerdir+'lua53-32.dll');
            addfile(yeXgineerdir+'win32\dbghelp.dll','win32');

            if cbSpeedhack.checked then
              addfile(yeXgineerdir+'speedhack-i386.dll');

            if cbvehdebug.checked then
              addfile(yeXgineerdir+'vehdebug-i386.dll');

            if cbKernelDebug.checked then
            begin
              addfile(yeXgineerdir+'dbk32.sys');
              addfile(yeXgineerdir+'dbk64.sys');
              addfile(yeXgineerdir+'eXgineer-i386.exe.sig');
            end;

            if cbModPlayer.checked then
              addfile(yeXgineerdir+'libmikmod32.dll');

          end
          else
          begin
            addfile(yeXgineerdir+'eXgineer-x86_64.exe');
            addfile(yeXgineerdir+'lua53-64.dll');

            if cbSpeedhack.checked then
              addfile(yeXgineerdir+'speedhack-x86_64.dll');

            if cbvehdebug.checked then
              addfile(yeXgineerdir+'vehdebug-x86_64.dll');

            if cbKernelDebug.checked then
            begin
              addfile(yeXgineerdir+'dbk64.sys');
              addfile(yeXgineerdir+'eXgineer-x86_64.exe.sig');
            end;

            if cbModPlayer.checked then
              addfile(yeXgineerdir+'libmikmod64.dll');

          end;

          if cbDotNet.checked then
          begin
            addfile(yeXgineerdir+'DotNetDataCollector32.exe');
            addfile(yeXgineerdir+'DotNetDataCollector64.exe');
          end;

          if cbD3DHook.checked then
          begin
            addfile(yeXgineerdir+'overlay.fx');
            if rb32.checked then
            begin
              addfile(yeXgineerdir+'d3dhook.dll');
              addfile(yeXgineerdir+'ced3d9hook.dll');
              addfile(yeXgineerdir+'ced3d10hook.dll');
              addfile(yeXgineerdir+'ced3d11hook.dll');
            end
            else
            begin
              addfile(yeXgineerdir+'d3dhook64.dll');
              addfile(yeXgineerdir+'ced3d9hook64.dll');
              addfile(yeXgineerdir+'ced3d10hook64.dll');
              addfile(yeXgineerdir+'ced3d11hook64.dll');
            end;
          end;
      ...
      showMessage(Format(rsTrainerFileMissing, [yeXgineerdir+basefile, filename]));
...
begin
  ...
  d:=ExtractRelativepath(yeXgineerdir, d);
...
begin
  ...
  OpenDialog1.InitialDir:=YeXgineerDir;
  SelectDirectoryDialog1.InitialDir:=YeXgineerDir;
