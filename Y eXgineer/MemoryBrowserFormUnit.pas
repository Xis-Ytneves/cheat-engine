...
type
  ...
  TMemoryBrowser = class(TForm)
    ...
    procedure miLuagineerClick(Sender: TObject);
...
uses Valuechange, MainUnit, debugeventhandler, findwindowunit,
  ...
  frmluagineerunit, disassemblerviewlinesunit, frmBreakpointConditionunit,
...
resourcestring
  ...
  rsYeXgineerSingleLingeAssembler = 'Single-line assembler';
  ...
  rsPleaseTargetAnotherProcess = 'Start another version of Y eXgineer and attach to that instead';
...
procedure TMemoryBrowser.miLuagineerClick(Sender: TObject);
begin
  //start lua gineer window
  if frmLuagineer=nil then
    frmLuagineer:=TfrmLuagineer.create(MemoryBrowser); //main mb
...
begin
  ...
  try
    if reg.OpenKey('\Software\Y eXgineer\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\',true) then
    begin
    ...
    if reg.OpenKey('\Software\Y eXgineer\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\Font',true) then
      SaveFontToRegistry(disassemblerview.font, reg);

    if reg.OpenKey('\Software\Y eXgineer\Hexview '+inttostr(screen.PixelsPerInch),true) then
    begin
      reg.writeInteger('spaceBetweenLines', hexview.spaceBetweenLines);
      reg.WriteBool('showStatusBar', hexview.statusbar.Visible);
    end;

    if reg.OpenKey('\Software\Y eXgineer\Hexview '+inttostr(screen.PixelsPerInch)+'\Font',true) then
      SaveFontToRegistry(hexview.hexfont, reg);

    if reg.OpenKey('\Software\Y eXgineer\RegisterView '+inttostr(screen.PixelsPerInch)+'\Font',true) then
...
begin
  ...
  try
    ...
    if reg.OpenKey('\Software\Y eXgineer\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\Font',false) then
    begin
      LoadFontFromRegistry(f, reg);
      disassemblerview.font:=f;
    end;

    if reg.OpenKey('\Software\Y eXgineer\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\',false) then
    begin
    ...
    if reg.OpenKey('\Software\Y eXgineer\Hexview '+inttostr(screen.PixelsPerInch),false) then
    begin
    ...
    if reg.OpenKey('\Software\Y eXgineer\Hexview '+inttostr(screen.PixelsPerInch)+'\Font',false) then
    begin
      LoadFontFromRegistry(f, reg);
      hexview.hexfont:=f;
    end;

    if reg.OpenKey('\Software\Y eXgineer\RegisterView '+inttostr(screen.PixelsPerInch)+'\Font',false) then
    begin
      LoadFontFromRegistry(f, reg);
//      scrollbox1.Font:=f;
...
//this will add the selected recdord to the list
  hexview.AddSelectedAddressToeXTable;
...
begin
  ...
  begin
    ...
    begin
      beep; //Best sound effect y eXgineer has
      exit;
    end;
  end;
...
//  copy
  ...
  assemblercode:=InputboxTop(rsYeXgineerSingleLingeAssembler, Format(rsTypeYourAssemblerCodeHereAddress, [inttohex(Address, 8)]), assemblercode, x='', canceled, assemblerHistory);
...
{will save a y eXgineer memory region file .CEM}
begin
