...
ret 
  ...
  if injectDLL(getYeXgineerDir()..[[\autorun\dlls\]]..dllname)==false then
...
function monoform_miExpandAllClick(sender)
  if messageDialog(translate("Are you sure you wish to expand the whole tree? This can take a while and Y eXgineer may look like it has crashed (It has not)"), mtConfirmation, mbYes, mbNo)==mrYes then
...
function mono_dissect()
  ...
  if (monoForm==nil) then
    monoForm=createFormFromFile(getYeXgineerDir()..[[\autorun\forms\MonoDataCollector.frm]])
...
function mono_initialize()
  ...
  if (mono_init1==nil) then
    ...
    registerEXETrainerFeature('Mono', function()
      local r={}
      r[1]={}
      r[1].PathToFile=getYeXgineerDir()..[[autorun\monoscript.lua]]
      r[1].RelativePath=[[autorun\]];

      r[2]={}
      r[2].PathToFile=getYeXgineerDir()..[[autorun\forms\MonoDataCollector.frm]]
      r[2].RelativePath=[[autorun\forms\]];

      r[3]={}
      r[3].PathToFile=getYeXgineerDir()..[[autorun\dlls\MonoDataCollector32.dll]]
      r[3].RelativePath=[[autorun\dlls\]];

      r[4]={}
      r[4].PathToFile=getYeXgineerDir()..[[autorun\dlls\MonoDataCollector64.dll]]
