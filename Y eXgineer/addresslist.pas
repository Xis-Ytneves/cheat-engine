...
type
  ...
  private
   ...
   // procedure TreeviewKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    function eXTableNodeHasOnlyAutoAssemblerScripts(eXTable: TDOMNode): boolean; //helperfunction
  ...
  public
    ...
    procedure refresh;
    procedure SelectAll;
    procedure saveTableXMLToNode(eXEntries: TDOMNode; selectedOnly: boolean=false);
    procedure loadTableXMLFromNode(eXEntries: TDOMNode);
...
procedure TAddresslist.saveTableXMLToNode(eXEntries: TDOMNode; selectedOnly: boolean=false);
var tn: TTreenode;
p: TTreenode;
m: TMemoryRecord;
begin
  ...
  while tn<>nil do
  begin
    TMemoryRecord(tn.data).getXMLNode(eXEntries,selectedonly);
...
procedure TAddresslist.loadTableXMLFromNode(eXEntries: TDOMNode);
var currentEntry: TDOMNode;
memrec: TMemoryRecord;
i: integer;
begin
  currentEntry:=eXEntries.FirstChild;
  while currententry<>nil do
  begin
    if tdomelement(currententry).TagName='eXEntry' then
    begin
...
begin
  ...
  eXtable:=doc.CreateElement('eXTable');
  doc.AppendChild(eXtable);

  eXEntries:=doc.CreateElement('eXEntries');
  eXtable.AppendChild(eXEntries);

  try
    saveTableXMLToNode(eXEntries, selectedOnly);
    WriteXMLFile(doc,ms);
    ms.WriteByte(0);
...
function TAddresslist.eXTableNodeHasOnlyAutoAssemblerScripts(eXTable: TDOMNode): boolean;
{
private
checks if the given xml document contains eXentries that aren't aa scripts
}
var eXEntries, currentEntry: TDOMNode;
  vt: TVariableType;
  vtnode: TDOMNode;
begin
  result:=true;
  //go through the list untill one is found that has the custom type
  eXEntries:=eXTable.FindNode('eXEntries');
  if eXentries<>nil then
  begin
    currentEntry:=eXEntries.FirstChild;
    while currententry<>nil do
    begin
          ...
          begin
            ...
            try
              ...
              begin
                ...
                if not eXTableNodeHasOnlyAutoAssemblerScripts(eXTable) then
