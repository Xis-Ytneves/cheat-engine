...
begin
  ...
  if launch32bit then
  begin
    if fileexists(selfpath+'eXgineer-i386.exe') then
      ShellExecute(0, 'open', pchar(selfpath+'eXgineer-i386.exe'), pchar(param), pchar(selfpath), sw_show)
    else
      MessageBox(0, 'eXgineer-i386.exe could not be found. Please disable/uninstall your anti virus and reinstall Y eXgineer to fix this','Y eXgineer launch error',MB_OK or MB_ICONERROR);
  end
  else
  begin
    if FileExists(selfpath+'eXgineer-x86_64.exe') then
      ShellExecute(0, 'open', pchar(selfpath+'eXgineer-x86_64.exe'), pchar(param), pchar(selfpath), sw_show)
    else
      MessageBox(0, 'eXgineer-x86_64.exe could not be found. Please disable/uninstall your anti virus and reinstall Y eXgineer to fix this','Y eXgineer launch error',MB_OK or MB_ICONERROR);
  end;
