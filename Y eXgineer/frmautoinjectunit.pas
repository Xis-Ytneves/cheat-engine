...
{
The TDisassemblyLine originates from jgoemat  ( http://forum.eXgineer.org/viewtopic.php?t=566415 )
...
type
  ...
  TfrmAutoInject = class(TForm)
    ...
    Exit1: TMenuItem;
    AssigntocurrenteXtable1: TMenuItem;
    emplate1: TMenuItem;
    Codeinjection1: TMenuItem;
    eXTablecompliantcodee1: TMenuItem;
    APIHook1: TMenuItem;
    SaveAs1: TMenuItem;
    ...
    procedure eXTablecompliantcodee1Click(Sender: TObject);

    procedure AssigntocurrenteXtable1Click(Sender: TObject);
...
uses frmAAEditPrefsUnit,MainUnit,memorybrowserformunit,APIhooktemplatesettingsfrm,
  ...
  frmLuagineerUnit, frmSyntaxHighlighterEditor;
...
resourcestring
  ...
  rsCEAFilter = 'Y eXgineer Assembly (*.CEA)|*.CEA|All Files ( *.* )|*.*';
  rsCEGAFilter = 'Y eXgineer GNU Assembly (*.CEGA)|*.CEGA|All Files ( *.* )|*.*';
...
begin
  ...
  case mode of
    ...
    begin
      ...
      AssigntocurrenteXtable1.visible:=false;
    ...
    begin
      ...
      AssigntocurrenteXtable1.Visible:=true;
    ...
    begin
      ...
      AssigntocurrenteXtable1.visible:=true; //yup
...
procedure TfrmAutoInject.eXTablecompliantcodee1Click(Sender: TObject);
var e,d: integer;
begin
{$ifndef standalonetrainerwithassembler}
  ...
  begin
    ...
    assemblescreen.Lines.Insert(1,'//code from here to ''[DISABLE]'' will be used to enable the eX');
  ...
  begin
    ...
    assemblescreen.Lines.Add('//code from here till the end of the code will be used to disable the eX');
  end;
{$endif}
end;
...
procedure TfrmAutoInject.AssigntocurrenteXtable1Click(Sender: TObject);
var a,b: integer;
    aa:TCEAllocArray;
    exceptionlist:TCEExceptionListArray;
    registeredsymbols: TStringlist;
...
begin
  ...
  begin
    ...
    try
      if reg.OpenKey('\Software\Y eXgineer\Auto Assembler\',false) then
      begin
...
{$ifndef standalonetrainerwithassembler}
 ...
 obsolete
  ...
  begin
    ...
    injectdll(yeXgineerdir+'undercdll.dll','');
  ...
  begin
    try
      ...
      begin
        ...
        reg:=tregistry.create;
        try
          if reg.OpenKey('\Software\Y eXgineer\Auto Assembler\',true) then
          begin
...
// \/   http://forum.eXgineer.org/viewtopic.php?t=566415 (jgoemat and some mods by db)
procedure GenerateFullInjectionScript(Script: tstrings; address: string);
var
  originalcode: array of string;
  originalbytes: array of byte;
...
begin
  ...
  // same as menu option "Y eXgineer framework code", make sure we
...
begin
  LuaHighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\Y eXgineer\Lua Highlighter');
  AAHighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\Y eXgineer\AA Highlighter');
end;
...
begin
  ...
  begin
    LuaHighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\Y eXgineer\Lua Highlighter');
    reloadHighlighterSettings;
...
begin
  ...
  if frmHighlighterEditor.showmodal=mrok then
  begin
    AAHighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\Y eXgineer\AA Highlighter');
