...
resourcestring
  ...
  strCantLoadFilepatcher='The file patcher can''t be loaded by Y eXgineer!';
  strNotACETrainer='This is not a trainer made by Y eXgineer (If it is a trainer at all!)';
  strUnknownTrainerVersion='This version of Y eXgineer doesn''t know how to read this trainer! Trainerversion=';
  ...
  rsThisIsNotAValideXTable = 'This is not a valid eX table';
  ...
  rsAskIfStupid = 'Generating a trainer with the current state of the eX '
    +'table will likely result in a completely useless trainer that does '
    +'nothing. Are you sure?';
  rsOSThereIsANewerVersionifYeXgineerOutEtc = 'There is a newer version of Y eXgineer out. It''s recommended to use that version instead';
  rsOSThiseXTableIsCorrupt = 'This eX table is corrupt';
...
begin
  ...
  try
    ...
    eXTable:=doc.FindNode('eXTable');
    if eXTable<>nil then
    begin

      signed:={$ifdef windows}isProperlySigned(TDOMElement(eXtable), signedstring, imagepos, image){$else}false{$endif};

      try
        tempnode:=eXTable.Attributes.GetNamedItem('YeXgineerTableVersion');
      except
        tempnode:=nil;
      end;
      ...
      begin
        try
          ...
          if (version>CurrentTableVersion) then
            showmessage(rsOSThereIsANewerVersionifYeXgineerOutEtc);

          lastLoadedTableVersion:=version;
        except
          showmessage(rsOSThiseXTableIsCorrupt);
        end;
      end;


      Files:=eXTable.FindNode('Files');
      Forms:=eXTable.FindNode('Forms');
      Entries:=eXTable.FindNode('eXEntries');
      Codes:=eXTable.FindNode('eXCodes');
      Symbols:=eXTable.FindNode('UserdefinedSymbols');
      Structures:=eXTable.FindNode('Structures');
      Comments:=eXTable.FindNode('Comments');
      LuaScript:=eXTable.FindNode('LuaScript');
      DComments:=eXTable.FindNode('DisassemblerComments');
    end;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          if Reg.OpenKey('\Software\Y eXgineer',false) then   //fill it from the registry (in case it's loaded before the settings are loaded)
          begin
...
begin
  ...
  try
    ...
    begin
      memfile.WriteBuffer(pchar('EXGINEER')^,11);
      temp:=2; //version
...
begin
  ...
  try
    ...
    begin
      ...
      if X='EXGINEER' then
      begin
         doc:=ConverteXTableToXML(filename)
      end
      else
      begin
        //protected
    ...
    try
      ...
      on e: exception do
        raise exception.Create(rsThisIsNotAValideXTable + ' ('+e.message+')');
    end;
...
begin
  ...
  eXTable:=TDOMElement(doc.AppendChild(TDOMNode(doc.CreateElement('eXTable'))));
  TDOMElement(eXTable).SetAttribute('YeXgineerTableVersion',IntToStr(CurrentTableVersion));

  if mainform.LuaForms.count>0 then
  begin
    Forms:=eXTable.AppendChild(doc.CreateElement('Forms'));
  ...
  begin
    Files:=eXTable.AppendChild(doc.CreateElement('Files'));
    for i:=0 to mainform.Luafiles.count-1 do
...
//        CodeRecord.Attributes.SetNamedItem(doc.CreateAttribute('GroupHeader')).TextContent:='1';
  ...
  try
    symbols:=eXTable.AppendChild(doc.CreateElement('UserdefinedSymbols'));
  ...
  begin
    Structures:=eXTable.AppendChild(doc.CreateElement('Structures'));
  ...
  if comments.memo1.Lines.Count>0 then
  begin
    comment:=eXTable.AppendChild(doc.CreateElement('Comments'));
  ...
  begin
    luascript:=eXTable.AppendChild(doc.CreateElement('LuaScript'));
  ...
  begin
    dcomments:=eXTable.AppendChild(doc.CreateElement('DisassemblerComments'));
...
begin
  ...
  begin
    //no eX header, so old bad storage method
