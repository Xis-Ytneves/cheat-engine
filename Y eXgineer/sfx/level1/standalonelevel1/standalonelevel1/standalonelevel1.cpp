			...
			if (i)
			{
			 ...
			 // STATUS=0x80070002;
			  if (!FAILED(STATUS))
			  {
				  ...
				  if (!correctversion)
				  {
					  MessageBoxA(0,"Please update your Y eXgineer version to Y eXgineer 6.3 or later\n", "Launch Error",MB_OK | MB_ICONERROR);
				  }
				  else
				  {
					  //launch it
					  ...
					  {						  
						  //Because Y eXgineer deletes files with name CET_TRAINER.CETRAINER it can be used to determine when ce is finished with it			  
			  ...
			  else
			  {
				  ...
				  else
				  {
					  ...
					  sprintf_s(errorstring, 254,"Your system must have Y eXgineer installed to be able to use this trainer\nwww.eXgineer.org\n(%x)", STATUS);
