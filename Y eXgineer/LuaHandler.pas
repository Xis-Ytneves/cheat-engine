...
uses autoassembler, mainunit, MainUnit2, LuaClass, frmluagineerunit, plugin, pluginexports,
  ...
  LuaRadioGroup, LuaRasterImage, LuaeXComponent, LuaAddresslist, byteinterpreter,
...
resourcestring
  ...
  rsyeXgineerIsBeingAFag = 'eXgineer is being a fag';
...
begin
  ...
  begin
    ...
    begin
      ...
      if (error<>'') then
      begin
        usesluagineerform:=false;
        if printoutput=nil then
        begin
          if frmLuagineer=nil then
            frmLuagineer:=TfrmLuagineer.Create(application);

          printoutput:=frmLuagineer.mOutput.Lines;
          usesluagineerform:=true;
        end;

        printoutput.add(rsError+error);

        if (frmLuagineer<>nil) and usesluagineerform and (frmLuagineer.cbShowOnPrint.checked) then
          frmLuagineer.show;

        if usesluagineerform then
          printoutput:=nil;
...
function lua_ToCEUserData(L: PLua_state; i: integer): pointer;
//Y eXgineer implements two types of userdata. the legacy LightUserData used in 6.2- and the Heavy UserData in 6.3+
...
begin
  ...
  begin
    ...
    r := FindFirst(YeXgineerDir+{$ifdef darwin}'m'+{$endif}'autorun'+pathdelim+'*.lua', FaAnyfile, DirInfo);
    while (r = 0) do
    begin
      ...
      begin
        ...
        begin
          ...
          i:=lua_dofile(luavm, pchar( UTF8ToWinCP(YeXgineerDir+{$ifdef darwin}'m'+{$endif}'autorun'+pathdelim+DirInfo.name)));
          if i<>0 then //error
          begin
            i:=lua_gettop(luavm);
            if i>0 then
          ...
          begin
            ...
            if lua_isnil(LuaVM,-1) then
            begin
              MessageDlg(format(rsScriptCorruptedVar, [YeXgineerDir+{$ifdef darwin}'m'+{$endif}'autorun'+pathdelim+DirInfo.name, 'MainForm']), mtError,[mbOK],0);
              mainformwasset:=false;
            end;
            lua_pop(LuaVM,1);
          end;
          ...
          begin
            ...
            if lua_isnil(LuaVM,-1) then
            begin
              MessageDlg(format(rsScriptCorruptedVar, [YeXgineerDir+{$ifdef darwin}'m'+{$endif}'autorun'+pathdelim+DirInfo.name, 'AddressList']), mtError,[mbOK],0);
              addresslistwasset:=false;
            end;
            lua_pop(LuaVM,1);
          end;
          ...
          begin
            case parameters[i].VType of
              ...
              system.vtWideChar, vtPWideChar, vtVariant, vtInterface,
                vtWideString: lua_pushstring(L, rsyeXgineerIsBeingAFag);
...
begin
  ...
  begin
    if frmLuagineer=nil then
      frmLuagineer:=TfrmLuagineer.Create(MemoryBrowser);
...
function supportYeXgineer(L: Plua_State): integer; cdecl;
var
  parameters: integer;
...
function fuckYeXgineer(L: Plua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(L));
  if adwindow<>nil then
...
function getYeXgineerDir(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(l));
  lua_pushstring(L, YeXgineerDir);
  result:=1;
end;

function lua_getYeXgineerProcessID(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(l));
...
function yeXgineerIs64Bit(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(L));
...
function getLuagineer(L:PLua_State): integer; cdecl;
begin
  if frmLuagineer=nil then
    frmLuagineer:=TfrmLuagineer.Create(application);

  luaclass_newClass(L, frmLuagineer);
  result:=1;
end;
...
function lua_getYeXgineerFileVersion(L: Plua_State): integer; cdecl;
begin
...
begin
  ...
  begin
    ...
    try
      cefuncproc.InjectDll(YeXgineerDir+'winhook-'+ts+'.dll');
    except
    end;
  end;
...
begin
  ...
  begin
    ...
    Lua_register(L, 'supportYeXgineer', supportYeXgineer);
    Lua_register(L, 'fuckYeXgineer', fuckYeXgineer);
    ...
    lua_register(L, 'getYeXgineerDir', getYeXgineerDir);
    lua_register(L, 'getYeXgineerProcessID', lua_getYeXgineerProcessID);
    ...
    lua_register(L, 'yeXgineerIs64Bit', yeXgineerIs64Bit);
    ...
    lua_register(L, 'getLuagineer', getLuagineer);
    ...
    lua_register(L, 'getYeXgineerFileVersion', lua_getYeXgineerFileVersion);
