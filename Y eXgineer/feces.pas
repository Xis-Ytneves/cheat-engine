unit feces;
//friends endorsing y eXgineer system
...
procedure signTable(eXtable: TDOMElement);
procedure signTableFile(f: string);
function isProperlySigned(eXtable: TDOMElement; out specialstring: string; out imagepos: integer; out image: tpicture): boolean;
...
resourcestring
  ...
  rsSelectYourYeXgineerSignatureFile = 'Select your y eXgineer signature '
    +'file';
  rsYeXgineerSignatureFiles = 'y eXgineer signature files';
  ...
  rsInvalidPublicKey = 'The provided public key is invalid(Not signed by the '
    +'Y eXgineer guy). Remove the signature section to load this table';
  ...
  rsFailedToLoadYeXgineerPublicKey = 'Failed to load y eXgineer public key';
...
var
  yeXgineerpublictablekey: BCRYPT_KEY_HANDLE=0;
...
procedure getXmlfileWithoutSignature(eXtable: TDOMElement; output: tstream);
var
  signature: TDOMNode;
begin
  signature:=eXTable.FindNode('Signature');

  if signature<>nil then
  begin
    eXTable.DetachChild(signature);
    signature.free;
  end;

  if eXtable<>nil then
  begin
    //showmessage(eXtable.TextContent);
    WriteXML(eXtable,output);
//    output.WriteAnsiString(eXtable.TextContent);
  end;
end;
...
var
  ...
  eXtablecontents: TMemorystream=nil;
...
begin
  ...
  try
    ...
    if yeXgineerpublictablekey=0 then
    begin
      s:=BCryptImportKeyPair(hAlgoritm, 0, BCRYPT_ECCPUBLIC_BLOB, yeXgineerpublictablekey, @publictablekey[0], 140, 0);
      if not succeeded(s) then raise exception.create(
        rsFailedToLoadYeXgineerPublicKey);
    end;
    ...
    s:=BCryptVerifySignature(yeXgineerpublictablekey,nil,hashbuffer,hashlength,sig, signaturesize,0);
    ...
    eXtablecontents:=TMemoryStream.create;
    getXmlfileWithoutSignature(eXtable, eXtablecontents);

    s:=BCryptCreateHash(hashAlgoritm, hHash, bHashObject, objectlength, nil, 0, 0);
    if not Succeeded(s) then raise exception.create(rsFailedCreatingHash2);

    s:=BCryptHashData(hHash, eXtablecontents.Memory, eXtablecontents.size, 0);
    ...
    if eXtablecontents<>nil then
      freeandnil(eXtablecontents);
  ...
  //check the signatureless version of the eX table with this public key
end;
...
begin
  ReadXMLFile(d, f);
  e:=TDOMElement(d.FindNode('eXTable'));
  signtable(e);
  WriteXMLFile(d,f);
end;
...
begin
  ...
  begin
    ...
    try
      od.Title:=rsSelectYourYeXgineerSignatureFile;
      od.Filter:=rsYeXgineerSignatureFiles+'|*.CESIG';
  ...
  try
    ...
    try
      ...
      //showmessage(pchar(eXtablecontents.Memory));
      ...
      signature:=eXTable.AppendChild(doc.CreateElement('Signature'));
