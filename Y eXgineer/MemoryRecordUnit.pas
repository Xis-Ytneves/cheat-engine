...
type
  ...
  public
    ...
    procedure setXMLnode(eXEntry: TDOMNode);
...
uses mainunit, addresslist, formsettingsunit, LuaHandler, lua, lauxlib, lualib,
  processhandlerunit, Parsers, {$ifdef windows}winsapi,{$endif}autoassembler, globals{$ifdef windows}, eXecoins{$endif};
{$endif}
...
procedure TMemoryRecord.setXMLnode(eXEntry: TDOMNode);
var
...
begin
  {$IFNDEF jni}
  if TDOMElement(eXEntry).TagName<>'eXEntry' then exit; //invalid node type

  tempnode:=eXentry.FindNode('ID');
  if tempnode<>nil then
    id:=strtoint(tempnode.textcontent);

  tempnode:=eXEntry.FindNode('Description');
  if tempnode<>nil then
  ...
  tempnode:=eXEntry.FindNode('Options');
  if tempnode<>nil then
  begin
  ...
  tempnode:=eXEntry.FindNode('DropDownListLink');
  if tempnode<>nil then
  begin
  ...
  tempnode:=eXEntry.FindNode('DropDownList');
  if tempnode<>nil then
  begin
  ...
  tempnode:=eXEntry.FindNode('ShowAsHex');
  if tempnode<>nil then
    fshowashex:=tempnode.textcontent='1';

  tempnode:=eXEntry.FindNode('ShowAsSigned');
  if tempnode<>nil then
  begin
  ...
  tempnode:=eXEntry.FindNode('Color');
  if tempnode<>nil then
  begin
    try
  ...
  tempnode:=eXEntry.FindNode('GroupHeader');
  if tempnode<>nil then
  begin
  ...
  tempnode:=eXEntry.FindNode('eXEntries');
  if tempnode<>nil then
  begin
  ...
  begin
    tempnode:=eXEntry.FindNode('VariableType');
    if tempnode<>nil then
    ...
    case VarType of
      vtCustom:
      begin
        tempnode:=eXEntry.FindNode('CustomType');
        if tempnode<>nil then
      ...
      vtBinary:
      begin
        tempnode:=eXEntry.FindNode('BitStart');
        if tempnode<>nil then
          extra.bitData.Bit:=strtoint(tempnode.TextContent);

        tempnode:=eXEntry.FindNode('BitLength');
        if tempnode<>nil then
          extra.bitData.bitlength:=strtoint(tempnode.TextContent);

        tempnode:=eXEntry.FindNode('ShowAsBinary');
        if tempnode<>nil then
      ...
      vtByteArray:
      begin
        tempnode:=eXEntry.FindNode('ByteLength');
        if tempnode<>nil then
      ...
      vtAutoAssembler:
      begin
        tempnode:=eXentry.FindNode('AssemblerScript');
    ...
    tempnode:=eXEntry.FindNode('Address');
    if tempnode<>nil then
    begin
    ...
    tempnode:=eXEntry.FindNode('Offsets');
    if tempnode<>nil then
    begin
    ...
    tempnode:=eXEntry.FindNode('Hotkeys');
...
var
  ...
  eXEntry: TDOMNode=nil;
  eXEntries: TDOMNode=nil;
  ...
  var i: integer;
  begin
    eXEntry.AppendChild(doc.CreateElement('Address')).TextContent:=interpretableaddress;
...
begin
  ...
  eXEntry.AppendChild(doc.CreateElement('ID')).TextContent:=IntToStr(ID);



  s:=utf8tosys(description);
  eXEntry.AppendChild(doc.CreateElement('Description')).TextContent:='"'+s+'"';

  //save options
  //(moHideChildren, moBindActivation, moRecursiveSetValue);
  if options<>[] then
  begin
    opt:=eXEntry.AppendChild(doc.CreateElement('Options'));
  ...
  if DropDownLinked then
  begin
    ddl:=eXEntry.AppendChild(doc.CreateElement('DropDownListLink'));
    ddl.TextContent:=DropDownLinkedMemrec;
  end
  else
  if DropDownList.Count>0 then
  begin
    ddl:=eXEntry.AppendChild(doc.CreateElement('DropDownList'));
  ...
  if Value<>'??' then
  begin
    laststate:=eXEntry.AppendChild(doc.CreateElement('LastState'));
  ...
  if showAsHex then
    eXEntry.AppendChild(doc.CreateElement('ShowAsHex')).TextContent:='1';

  if fShowAsSignedOverride then
  begin
    if fShowAsSigned then
      eXEntry.AppendChild(doc.CreateElement('ShowAsSigned')).TextContent:='1'
    else
      eXEntry.AppendChild(doc.CreateElement('ShowAsSigned')).TextContent:='0';
  end;


  if fcolor<>clWindowText then
    eXEntry.AppendChild(doc.CreateElement('Color')).TextContent:=inttohex(fcolor,6);
  ...
  begin
    eXEntry.AppendChild(doc.CreateElement('VariableType')).TextContent:=VariableTypeToString(vartype);
    case VarType of
      vtCustom:
      begin
        eXentry.AppendChild(doc.CreateElement('CustomType')).TextContent:=CustomTypeName;
      end;

      vtBinary:
      begin
        eXEntry.AppendChild(doc.CreateElement('BitStart')).TextContent:=inttostr(extra.bitData.Bit);
        eXEntry.AppendChild(doc.CreateElement('BitLength')).TextContent:=inttostr(extra.bitData.BitLength);
        eXEntry.AppendChild(doc.CreateElement('ShowAsBinary')).TextContent:=BoolToStr(extra.bitData.showasbinary,'1','0');
      end;

      vtString:
      begin
        eXEntry.AppendChild(doc.CreateElement('Length')).TextContent:=inttostr(extra.stringData.length);
        eXEntry.AppendChild(doc.CreateElement('Unicode')).TextContent:=BoolToStr(extra.stringData.unicode,'1','0');
        eXEntry.AppendChild(doc.CreateElement('CodePage')).TextContent:=BoolToStr(extra.stringData.codepage,'1','0');
        eXEntry.AppendChild(doc.CreateElement('ZeroTerminate')).TextContent:=BoolToStr(extra.stringData.ZeroTerminate,'1','0');

      end;

      vtByteArray:
      begin
        eXEntry.AppendChild(doc.CreateElement('ByteLength')).TextContent:=inttostr(extra.byteData.bytelength);
      end;

      vtAutoAssembler:
      begin
        n:=eXEntry.AppendChild(doc.CreateElement('AssemblerScript'));
        if fAsync then
        begin
  ...
  begin
    hks:=eXentry.AppendChild(doc.CreateElement('Hotkeys'));
    for i:=0 to HotkeyCount-1 do
    begin
  ...
  begin
    eXEntries:=doc.CreateElement('eXEntries');
    tn:=treenode.GetFirstChild;
    while tn<>nil do
    begin
      TMemoryRecord(tn.data).getXMLNode(eXEntries, false); //take over ALL attached nodes, not just the selected ones
      tn:=tn.GetNextSibling;
    end;

    eXentry.AppendChild(eXEntries);
  end;


  node.AppendChild(eXEntry);
 {$ENDIF}
end;
...
begin
  {$ifdef windows}
  if state and aprilfools then decreaseeXECoinCount;
  {$endif}
